...
client.on("roleDelete", async channel => {
    ...
    if (!anti[channel.guild.id + entry.id]) {
        ...
        if (anti[channel.guild.id + entry.id].actions >= config[channel.guild.id].roleDelLimit) {
            channel.guild.members.get(entry.id).ban().catch(e => channel.guild.owner.send(`**This user tried to remove many roles {entry.username}**`))
            anti[channel.guild.id + entry.id].actions = "0"
            fs.writeFile("./config.json", JSON.stringify(config, null, 2), function (e) {
                if (e) throw e;
            });
...
client.on("roleCreate", async channel => {
    ...
    if (!anti[channel.guild.id + entry.id]) {
        ...
        if (anti[channel.guild.id + entry.id].actions >= config[channel.guild.id].roleCrLimits) {
            channel.guild.members.get(entry.id).ban().catch(e => channel.guild.owner.send(`**This user tried to make many roles ${entry.username} **`))
            anti[channel.guild.id + entry.id].actions = "0"
            fs.writeFile("./config.json", JSON.stringify(config, null, 2), function (e) {
                if (e) throw e;
            });
...
client.on("guildBanAdd", async (guild, user) => {
    ...
    if (!anti[guild.id + entry.id]) {
        ...
        if (anti[guild.id + entry.id].actions >= config[guild.id].banLimit) {
            ...
            anti[guild.id + entry.id].actions = "0"
            fs.writeFile("./antigrief.json", JSON.stringify(config, null, 2), function (e) {
                if (e) throw e;
            });
            fs.writeFile("./antigrief.json", JSON.stringify(anti, null, 2), function (e) {
                if (e) throw e;
